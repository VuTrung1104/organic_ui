import { Leaf, MapPin, Phone, Mail, Clock } from 'lucide-react';
import Link from 'next/link';

export default function Footer() {
  return (
    <footer className="bg-gray-900 text-gray-300 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          {/* Company Info */}
          <div>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-green-600 rounded-full flex items-center justify-center">
                <Leaf className="w-7 h-7 text-white" />
              </div>
              <span className="text-2xl font-bold text-white" style={{fontFamily: 'cursive'}}>
                TrungOrganic
              </span>
            </div>
            <p className="text-base leading-relaxed text-gray-400">
              Cung cấp rau củ quả tươi ngon, sạch sẽ, an toàn cho sức khỏe gia đình bạn.
            </p>
            
            {/* Social Media Links */}
            <div className="flex gap-3 mt-6">
              <a 
                href="https://www.facebook.com/" 
                target="_blank" 
                rel="noopener noreferrer"
                className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center hover:bg-green-700 transition"
                aria-label="Facebook"
              >
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              <a 
                href="https://www.instagram.com" 
                target="_blank" 
                rel="noopener noreferrer"
                className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center hover:bg-green-700 transition"
                aria-label="Instagram"
              >
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              <a 
                href="https://www.youtube.com/watch?v=_XX248bq6Pw" 
                target="_blank" 
                rel="noopener noreferrer"
                className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center hover:bg-green-700 transition"
                aria-label="YouTube"
              >
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                </svg>
              </a>
              <a 
                href="tel:0356xxxxxx" 
                className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center hover:bg-green-700 transition"
                aria-label="Phone"
              >
                <Phone className="w-5 h-5 text-white" />
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="text-white text-lg font-bold mb-6">Liên Kết Nhanh</h4>
            <ul className="space-y-3 text-base text-gray-400">
              <li><Link href="/" className="hover:text-green-500 transition">Trang Chủ</Link></li>
              <li><Link href="/about" className="hover:text-green-500 transition">Giới Thiệu</Link></li>
              <li><Link href="/products" className="hover:text-green-500 transition">Sản Phẩm</Link></li>
              <li><Link href="/" className="hover:text-green-500 transition">Tin Tức</Link></li>
              <li><Link href="/contact" className="hover:text-green-500 transition">Liên Hệ</Link></li>
            </ul>
          </div>

          {/* Customer Support */}
          <div>
            <h4 className="text-white text-lg font-bold mb-6">Hỗ Trợ Khách Hàng</h4>
            <ul className="space-y-3 text-base text-gray-400">
              <li><a href="#" className="hover:text-green-500 transition">Chính Sách Đổi Trả</a></li>
              <li><a href="#" className="hover:text-green-500 transition">Hướng Dẫn Đặt Hàng</a></li>
              <li><a href="#" className="hover:text-green-500 transition">Phương Thức Thanh Toán</a></li>
              <li><a href="#" className="hover:text-green-500 transition">Câu Hỏi Thường Gặp</a></li>
              <li><a href="#" className="hover:text-green-500 transition">Điều Khoản Dịch Vụ</a></li>
            </ul>
          </div>

          {/* Contact Info */}
          <div>
            <h4 className="text-white text-lg font-bold mb-6">Thông Tin Liên Hệ</h4>
            <ul className="space-y-4 text-base text-gray-400">
              <li className="flex items-center gap-4">
                <MapPin className="w-5 h-5 text-green-500 shrink-0 mt-0.5" />
                <span className="leading-relaxed">
                  Đường 106, Thủ Đức, TP. Hồ Chí Minh
                </span>
              </li>
              <li className="flex items-center gap-4">
                <Phone className="w-5 h-5 text-green-500 shrink-0" />
                <a href="tel:0123456789" className="hover:text-green-500 transition">
                  03564000000
                </a>
              </li>
              <li className="flex items-center gap-4">
                <Mail className="w-5 h-5 text-green-500 shrink-0" />
                <a href="mailto:info@raucutuoi.vn" className="hover:text-green-500 transition">
                  trung@organic.vn
                </a>
              </li>
              <li className="flex items-center gap-4">
                <Clock className="w-5 h-5 text-green-500 shrink-0 mt-0.5" />
                <div>
                  <div className="font-semibold">Thứ 2 - Chủ Nhật</div>
                  <div>7:00 - 21:00</div>
                </div>
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-gray-800 pt-6 text-start">
          <p className="text-base">© 2025 TrungOrganic.</p>
        </div>
      </div>
    </footer>
  );
}
